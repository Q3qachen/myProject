		...
		methods: {
			...
			getConfirm: function() {
				...
				orderConfirm(data).then(res => {
					...
					that.confirm.forEach(item => {
						// 自动赋值：真实姓名
						if (item.label == 'text' && item.title == '真实姓名') {
							item.value = res.data.userInfo.real_name || ''
						}
						// 自动赋值：身份证号
						if (item.label == 'text' && item.title == '身份证号') {
							item.value = res.data.userInfo.card_id || ''
						}
					})
