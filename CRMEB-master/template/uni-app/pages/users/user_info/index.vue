			...
			<view class='personal-data' :style="colorStyle">
				<view class='list'>
					...
					<view class='item acea-row row-between-wrapper'>
					    <view>{{$t(`真实姓名`)}}</view>
					    <view class='input'>
					        <input type='text' name='real_name' :maxlength="10" :value='userInfo.real_name'></input>
					    </view>
					</view>
					...
					<view class='item acea-row row-between-wrapper'>
					    <view>{{$t(`身份证号`)}}</view>
					    <view class='input'>
					        <input type='idcard' name='card_id' :maxlength="18" :value='userInfo.card_id'></input>
					    </view>
					</view>
					<view class='item acea-row row-between-wrapper'>
						<view>{{$t(`手机号码`)}}</view>
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				if (!value.real_name) return that.$util.Tips({
					title: that.$t(`请输入真实姓名`)
				});
			
				// 新增验证：身份证不能为空 + 格式验证
				if (!value.card_id) return that.$util.Tips({
					title: that.$t(`请输入身份证号码`)
				});
				// 身份证正则校验（支持18位 / 15位）
				let idCardReg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
				if (!idCardReg.test(value.card_id)) {
					return that.$util.Tips({
						title: that.$t(`身份证号码格式不正确`)
					});
				}
