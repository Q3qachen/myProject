...
class Captcha extends \think\addons\Controller
{
    ...
    public function check()
    {
        ...
        $result = WechatCaptcha::check($captcha, $event, null, false);
        if ($result) {
            $this->success("验证码正确");
        } else {
            $this->error("验证码错误");
    ...
    public function send()
    {
        ...
        $params = [
            ...
            'action_info'    => [
                'scene' => [
                    'scene_str' => "captcha_" . $event . "_" . $ip . "_" . $context,
                ]
            ],
        ];
        //获取验证码
        ...
        if ($result['ret']) {
            ...
            if (isset($msg['ticket']) && isset($msg['url'])) {
                \think\Cookie::set('captcha_' . $event, $context);
