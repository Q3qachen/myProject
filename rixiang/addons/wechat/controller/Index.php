        ...
        $this->app->server->push(function ($message) {
            ...
            switch ($message['MsgType']) {
                case 'event': //事件消息
                    ...
                    //验证码消息
                    if (in_array($event, ['subscribe', 'SCAN']) && preg_match("/^(qrscene_)?captcha_([a-zA-Z0-9]+)_([0-9\.]+)_([0-9a-zA-Z]{6})/", $eventkey, $matches)) {
                        return WechatCaptcha::send($openid, $matches[2], $matches[3], $matches[4]);
                    }
                    switch ($event) {
                        case 'subscribe'://添加关注
                ...
                default: //其它消息
                    ...
                    if ($message['MsgType'] == 'text') {
                        $autoreply = null;
                        $autoreplyList = WechatAutoreply::where('status', 'normal')->field('id,text,eventkey')->order('weigh DESC,id DESC')->select();
                        try {
