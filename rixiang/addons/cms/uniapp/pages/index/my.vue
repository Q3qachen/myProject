	...
	<view>
		...
		<view class="u-p-t-30 u-p-b-50 home" :style="[{ backgroundColor: theme.bgColor || '#374486' }]">
			<view class="userinfo" :style="[{ height: vuex_user.vip > 0 ? '370rpx' : '310rpx' }]">
				<block v-if="vuex_token">
					<u-avatar size="120" :show-sex="vuex_user.vip == 0" :show-level="vuex_user.vip > 0" :sex-icon="vuex_user.gender == 1 ? 'man' : 'woman'" :src="vuex_user.avatar" @click="goPage('/pages/my/profile', true)"></u-avatar>
					...
					<view class="u-m-t-15" v-if="vuex_user.vip > 0">
						<u-tag type="success" :text="vuex_user.vipInfo && vuex_user.vipInfo.name ? vuex_user.vipInfo.name : ''" mode="light" shape="circle" size="mini" />
					</view>
			...
			<view class="corrugated">
				...
				<view class="wave-middle wave-item" :style="[{ backgroundImage: 'url(' + wavemiddle + ')', height: vuex_user.vip > 0 ? '230rpx' : '200rpx' }]"></view>
				<view class="wave-bottom wave-item" :style="[{ backgroundImage: 'url(' + wavebottom + ')', height: vuex_user.vip > 0 ? '230rpx' : '200rpx' }]"></view>
			</view>
		</view>
		<!-- 统计 -->
		...
		<view class="u-m-t-30">
			<u-cell-group>
				...
				<u-cell-item icon="level" title="我的VIP会员" @click="goPage('/pages/my/vip', true)"></u-cell-item>
				...
				<!-- <u-cell-item icon="man-add-fill" title="绑定账号" @click="goPage('/pages/my/bind', true)"></u-cell-item> -->
				...
				<u-cell-item icon="backspace" v-if="vuex_token" title="退出登录" @click="goOut()"></u-cell-item>
			</u-cell-group>
		</view>
	...
	export default {
		...
		methods: {
			...
			goPage: async function(path, auth) {
				...
				if (path == '/pages/my/vip' && !this.vuex_user.is_install_vip) {
