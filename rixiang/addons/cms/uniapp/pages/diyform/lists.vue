		...
		<view class="u-p-30">
			<view class="form-list">
				<view class="item u-border-top u-p-30 u-m-b-30 u-bg-white" v-for="(item, index) in list" :key="index" @click="goDetail(item.id)">
					<view class="u-m-b-10 u-text-weight title u-flex">
						<view class="u-m-r-10" v-if="item.status == 'hidden'">
							<u-tag :text="item.status_text" type="error" size="mini" />
						</view>
						<text v-text="item.title ? item.title : item.name"></text>
					</view>
