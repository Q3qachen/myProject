...
let i = 0, j = 0, k = 0;
for (const pk in citypickData["86"]) {
    provinces[i] = {label: citypickData["86"][pk], value: pk};
    j = 0;
    citys[i] = [];
    areas[i] = [];
    for (const ck in citypickData[pk]) {
        citys[i][j] = {label: citypickData[pk][ck], value: ck};
        k = 0;
        areas[i][j] = [];
        for (const dk in citypickData[ck]) {
            areas[i][j][k] = {label: citypickData[ck][dk], value: dk};
            k++;
        }
        j++;
    }
